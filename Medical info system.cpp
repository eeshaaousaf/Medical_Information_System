#include <iostream>
#include <fstream>  // csv form
#include <regex>  // to validate input
#include <string>  
#include <sstream>  
#include <ctime>  
...
string getValidatedInput(const string& prompt, const regex& pattern, const string& errorMessage) {
    ...
    while (true) {
        cout << prompt;
        getline(cin, input);   //getline  -> string  rege_match ->regex
...
int getValidatedInteger(const string& prompt, const string& errorMessage) {
    ...
    while (true) {
        ...
        getline(cin, input);
        try {
            int value = stoi(input); //  stoi -> sstream
            if (value > 0 && value< 150) return value; // Valid positive integer
        }
        catch (...) {
...
int getCurrentYear() {
    time_t t = time(0);  // Get current time(world time)
    struct tm now;       // Create a tm structure to hold local time(pc time)
...
public:
    ...
    void display() const {
        cout << "ID: " << id
            ...
            << ", Doctor ID: " << doctorID
            << ", Date: " << date
            << ", Time: " << time << endl; // Ensure time is displayed
    }
};
...
public:
    ...
    void addPatient() {
        ...
        regex phonePattern(R"(^\+?\d{2}[\s-]?\d{4}[\s-]?\d{5}$)"); // Phone validation pattern
        regex namePattern(R"([A-Za-z\s]+)");
        
        string name = getValidatedInput("Enter Patient Name: ", namePattern, "Invalid name. Only letters and spaces are allowed.");
